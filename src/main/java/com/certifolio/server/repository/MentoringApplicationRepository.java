...
import com.certifolio.server.domain.ApplicationStatus;
import com.certifolio.server.domain.MentoringApplication;
...
public interface MentoringApplicationRepository extends JpaRepository<MentoringApplication, Long> {
    ...
    // 특정 상태의 신청 목록 (멘토용)
    @Query("SELECT a FROM MentoringApplication a WHERE a.mentor.id = :mentorId AND a.status = :status ORDER BY a.createdAt DESC")
    List<MentoringApplication> findByMentorIdAndStatus(
